...
from .odoo_provider import OdooProvider
...
class OdooSyncManager:
    ...
    async def sync_users(self):
        ...
        await self.sync_users_from_odoo()
        await self.sync_users_to_odoo()

    async def sync_users_from_odoo(self):
        users = await self.repo.get_users()
        partners = await validate_partners(
        ...
        for partner in partners:
            ...
            if partner.email:
                ...
                odoo_user = await self.repo.get_partner(partner.id)
                if not odoo_user:
                    email = partner['email']
                    ...
                    if ordercast_partner:
                        self.logger.warning(f"User with email {email} already exists, ignoring.")
                        ...
                        if existing_odoo_user:
                            ...
                            await self.repo.update_partner(is_removed=False)
                            ...
                            await self.repo.upsert_user(user_id=ordercast_partner.id, odoo_id=partner["id"])
                    else:
                        name = partner['name']
                        defaults = {"name": name, "erp_id": partner["id"], "is_approved": False, "is_active": True,
                                    "is_removed": False, "password": secrets.token_urlsafe(nbytes=64),
                                    "status": UserStatus.NEW}

                        # saved, is_new = User.all_objects.update_or_create(email=email, defaults=defaults)
                        saved = await self.ordercast_api.upsert_user(email=email, defaults=defaults)

                        # UserExternal.all_objects.update_or_create(user_id=saved.id, odoo_id=partner["id"],
                        #                                           defaults={'is_removed': False})
                        await self.repo.save_user(user_id=saved.id, odoo_id=partner["id"])

                        partner['saved_id'] = saved.id
                        partner['saved'] = saved
                        lang = partner["language"] if "language" in partner and partner["language"] and len(
                            partner["language"]) == 2 else "fr"
                        ...
                        #                                                                  defaults=defaults)
                        user_profile = await self.ordercast_api.create_user_profile(user_id=saved.id,
                                                                                    defaults=defaults)
    ...
    def save_billing_address(self, billing_address, saved):
        if billing_address and saved:
            ...
            billing_info, _ = BillingInfo.objects.update_or_create(enterprise_name=company_name, address_id=address.id,
                                                                   vat=vat)
            billing, _ = Billing.objects.update_or_create(name=name, user_id=saved.id, billing_info_id=billing_info.id)
            return billing
    ...
    def save_address(self, address, user):
        if address and user:
            ...
            if is_not_empty(address, 'city'):
                ...
                city = await self.ordercast_api.get_city(address['city'])
                if city:
            ...
            if is_not_empty(address, 'country_code'):
                ...
                country = await self.ordercast_api.get_country(code2=address['country_code'])
                if country:
            ...
            address = await self.ordercast_api.create_address(user_id=user.id, name=name, defaults=defaults_address)
            # AddressExternal.objects.update_or_create(address_id=saved.id, odoo_id=address['id'])
            await self.repo.upsert_address(address_id=address.id, odoo_id=address['id'])
            return address

    async def sync_users_to_odoo(self):
...
def get_odoo_sync_manager(
    ...
    return OdooSyncManager(odoo_repo)
